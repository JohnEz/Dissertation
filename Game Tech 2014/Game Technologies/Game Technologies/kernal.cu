...
__device__ void cudaPatrol(Players* players, Agents* agents, short* agentsPartitions, short* partitionsPlayers, float msec, const unsigned int size)
{
	...
	while (i < 8 && agentsPartitions[((a)*8) + i] != -1)
	{
		...
		while (j < players->MAXPLAYERS && partitionsPlayers[partPlayer+j] != -1)
		{
			//the player
...
cudaError_t cudaUpdateAgents2(Players* players, Agents* agents, const unsigned int size, float msec, AIWorldPartition* partitions, const int partitionCount, Vector3* halfDim)
{
