...
public:
	...
	bool		GetAtRest() const			{ return atRest;}
	...
	void	SetAtRest(bool v)		{ atRest = v;}
	...
	int GetCurrentCol() { return currentCollisions; }

	void AddCol(int i) {currentCollisions += i; }
	void SetMaxCol(int m) { maxCollisions = m; }

	//Movement
	void explicitEuler(Vector3 acc, float dT);
	void implicitEuler(Vector3 acc, float dT);
	void symplecticEuler(Vector3 acc, float dT);
